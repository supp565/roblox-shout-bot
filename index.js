...
// FORCE NOBLOX TO USE ROPROXY GLOBALLY
// This intercepts noblox's internal request module and swaps out the domain name
const original_noblox_http = noblox.http;
noblox.http = function(options) {
    if (options && options.url) {
        options.url = options.url.replace('roblox.com', 'roproxy.com');
    }
    return original_noblox_http.apply(this, arguments);
};

async function startBot() {
    try {
        console.log("Initializing session via RoProxy interception...");
        ...
        console.log("Logged in. Attempting group shout via RoProxy routing agent...");

        // Fire standard shout method
        await noblox.shout(groupId, shoutMessage);
        console.log(`Successfully shouted: "${shoutMessage}"!`);
        
    } catch (error) {
        console.error("Bot failed to execute:", error.message || error);
    }
}
